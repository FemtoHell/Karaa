...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  ...
  router.get('/google/callback', (req, res) => {
    res.redirect(`${process.env.CLIENT_URL}/login?error=google_oauth_not_configured`);
  });
}

// LinkedIn OAuth routes (Only if credentials are provided)
if (process.env.LINKEDIN_CLIENT_ID && process.env.LINKEDIN_CLIENT_SECRET) {
  ...
  // Return error if LinkedIn OAuth is not configured
  router.get('/linkedin', (req, res) => {
    res.status(503).json({
      success: false,
      message: 'LinkedIn OAuth is not configured. Please contact administrator.'
    });
  });
  router.get('/linkedin/callback', (req, res) => {
    res.redirect(`${process.env.CLIENT_URL}/login?error=linkedin_oauth_not_configured`);
  });
}
