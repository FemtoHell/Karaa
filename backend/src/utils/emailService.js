...
// Initialize Resend (priority) or fallback to SMTP
let resend = null;
let useResend = false;
...
if (process.env.RESEND_API_KEY) {
  ...
  useResend = true;
  console.log('✅ Resend email service initialized (production mode)');
} else if (process.env.SMTP_EMAIL && process.env.SMTP_PASSWORD) {
  console.log('⚠️  Using SMTP email service (development mode)');
} else {
  console.error('❌ No email service configured! Set RESEND_API_KEY or SMTP credentials');
}

// Create SMTP transporter (fallback for local development)
...
// Send verification email via Resend or SMTP
